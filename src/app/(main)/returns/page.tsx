...
export default function ReturnsPage() {
  const [data, setData] = useState({ title: 'Returns & Refunds', content: '' })

  useEffect(() => {
    fetch('/api/settings')
      .then(r => r.json())
      .then((settings: any[]) => {
        const val = settings.find((s: any) => s.key === 'page_returns')?.value
        if (val) {
          try {
            const parsed = JSON.parse(val)
            setData({ title: parsed.title || 'Returns & Refunds', content: parsed.content || '' })
          } catch {}
        }
      })
      .catch(() => {})
      ...
      <div className="max-w-3xl mx-auto px-4">
        <div className="text-center mb-8">
          <div className="w-16 h-16 bg-[#f0d6de] rounded-full flex items-center justify-center mx-auto mb-4">
            <RotateCcw size={32} className="text-[#d4869c]" />
          </div>
